import factory
from tikiti.Task.models import Sector, Source, Support, Status, Issue, Priority, Task, TaskHistory, TaskFiles
...
class SupportFactory(factory.django.DjangoModelFactory):
    class Meta :
        model =Support
    support_type = factory.Faker("random_element",elements=['Fiber-Cutf','Los2','Slow-0','BLink','Test'])
    slug = factory.LazyFunction(uuid.uuid4)
    user = factory.SubFactory(UserFactory)
    description = factory.Faker("paragraph")
    create_date = factory.LazyFunction(datetime.datetime.now)
...
class StatusFactory(factory.django.DjangoModelFactory):
    class Meta :
        model =Status
    status = factory.Faker('word')#factory.Faker("random_element",elements=['New','Working On it','REsolved','Closed','Stop'])
    slug = factory.LazyFunction(uuid.uuid4)
    user = factory.SubFactory(UserFactory)
    description = factory.Faker("paragraph")
    create_date = factory.LazyFunction(datetime.datetime.now)
...
class TaskFactory(factory.django.DjangoModelFactory):
    ...
    assigned_to = factory.LazyFunction(lambda: Assignees.objects.first() or AssigneeFactory())
    create_date = factory.LazyFunction(datetime.datetime.now)
    update_date = factory.LazyFunction(datetime.datetime.now)